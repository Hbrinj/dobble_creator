...
| alpha-aware-packing | feature/alpha-aware-packing | Compute per-upload silhouette circle (Welzl's SEC over alpha-mask contour) and map it to each slot circle in `drawCard`; bump `PACKING_FRACTION` 0.55 → 0.65 to recover slot-edge space — stops adjacent card images from overlapping or clipping | In Review | — |
