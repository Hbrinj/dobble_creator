...
| readme-install | feature/readme-install | Add top-level `README.md` with project description, Node 20 prerequisite, install / run commands, dev-script table, and pointer to `ARCHITECTURE.md` | In Review | — |
| card-edge-margin | feature/card-edge-margin | Add configurable inner margin (`cardEdgeMarginMm`, default 5 mm, range 0–15) that keeps every image silhouette inside a no-image ring at the card's outer edge; packer-aware via boundary `1 - insetFraction` plus `PACKING_FRACTION * (1 - insetFraction)²` compensation in `src/lib/packer.ts`, threaded from `PrintSettings` through `computeInsetFraction` in `src/render/drawCard.ts` | In Review | — |
